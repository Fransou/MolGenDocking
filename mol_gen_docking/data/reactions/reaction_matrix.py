...
class ReactantReactionMatrix:
    ...
    def _init_matrix(
        self, matrix: np.ndarray | os.PathLike | None, batch_size: int = 1024
    ) -> np.ndarray:
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tempdir_s:
            ...
            joblib.Parallel(n_jobs=joblib.cpu_count() // 2)(
                ...
                for start in tqdm(
                    range(0, len(self._reactants), batch_size), desc="Create matrix"
                )
            )
    ...
    def seed_reaction_indices(self) -> list[int]:
        ...
        return list(
            np.nonzero(full_flag == np.bitwise_or.reduce(self._matrix, axis=0))[
                0
            ].tolist()
        )
    ...
    def reactant_count(self) -> np.ndarray:
        out = (self._matrix != 0).astype(np.int32).sum(0)
